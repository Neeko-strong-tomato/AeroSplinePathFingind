...
def segmentation(mesh_path: str, angle_deg: float = 30.0):
    ...
    for part in parts:
        ...
        # Trivial case: single segment
        if edges is None or len(edges) == 0 or n_faces < 2:
            segments.append(part.copy())
            continue
        ...
        # 🔹 Extract one mesh per segment
        for k in range(seg_id):
            face_idx = np.where(labels == k)[0]
            if len(face_idx) == 0:
                continue

            sub = part.submesh([face_idx], append=True, repair=False)
            if not sub.is_empty:
                segments.append(sub)
