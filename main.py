...
# mesh, labels, nseg = segmentation("fan.stl", angle_deg=30)
...
mesh, labels, nseg = segmentation("test_part_m.stl", angle_deg=30)
print("TEST - Segments:", nseg)
print("TEST - Faces:", len(mesh.faces))

show_segmentation(mesh, labels, title="test - 30deg")
exported = export_segmentation(
    mesh,
    labels,
    out_dir="out_test_segments",
    prefix="test_seg",
    min_faces=1,
    reindex_vertices=True,
    binary=True
)
print("TEST - Nb STL exportés:", len(exported))